Ext.define('Tscrm.view.__TABLEIZE__.Controller', {
    extend: 'Ext.app.ViewController',
    alias: 'controller.__LCFIRST__Controller', 
/*
...
*/
    reloadGrid:function(){
        ...
        this.getViewModel().getStore('__LCFIRST__List').load({
            scope: this,
...
*/      
    onRowDblClick: function (grid, record, index, eOpts) {
        win = new Tscrm.view.__TABLEIZE__.View();
...
*/
    loadViewData: function(controller,id,win){
        ...
        if(id!='')
        {
            ...
            var data = Ext.data.StoreManager.lookup('__CLASSNAME__View').load({
                params:{
                    'id': id
                },
                callback: function(records, operation, success) {
...
*/     
    onAdd: function(){
        win = Ext.widget('__LCFIRST__Form').show();
...
*/       
    onEdit: function(view, rowIdx, colIdx, item, e, record, row){
        ...
        var formPanel = win.lookupReference('__LCFIRST__Form');
        formPanel.mask("Adatok betöltése..."); 
        var form = formPanel.getForm(); 
               
        form.url='/admin/__TABLEIZE__/edit.json';
        var data = Ext.data.StoreManager.lookup('__CLASSNAME__View').load({
            params:{
                'id': record.data.id
            },
    ...
    onDelete: function(view, rowIdx, colIdx, item, e, record, row){
        ...
        Ext.Msg.show({
            ...
            fn: function(btn) {
                if (btn === 'yes') {   
                    Ext.Ajax.request({
                        url: '/admin/__TABLEIZE__/delete.json',
                        params:{
                            id:record.data.id
                        },
                        methid: 'POST',
...
*/        
    onFormCancel: function() {
        this.lookupReference('__LCFIRST__Form').getForm().reset();
        this.getView().close();
    },
/*
    Form save function
*/      
    onFormSave: function() {
        ...
        var vcontroller = this;
        var formPanel = this.lookupReference('__LCFIRST__Form');
