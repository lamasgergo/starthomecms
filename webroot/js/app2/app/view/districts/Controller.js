Ext.define('Tscrm.view.districts.Controller', {
    extend: 'Ext.app.ViewController',
    alias: 'controller.districtsController', 
/*
...
*/
    reloadGrid:function(){
        ...
        this.getViewModel().getStore('districtsList').load({
            scope: this,
...
*/
    loadViewData: function(controller,id,win){
        ...
        if(id!='')
        {
            ...
            var data = Ext.data.StoreManager.lookup('DistrictsView').load({
                params:{
                    'id': id
                },
                callback: function(records, operation, success) {
...
*/     
    onAdd: function(){
        win = Ext.widget('districtsForm').show();
...
*/       
    onEdit: function(view, rowIdx, colIdx, item, e, record, row){
        ...
        var formPanel = win.lookupReference('districtsForm');
        formPanel.mask("Adatok betöltése..."); 
        var form = formPanel.getForm(); 
               
        form.url='/admin/districts/edit.json';
        var data = Ext.data.StoreManager.lookup('DistrictsView').load({
            params:{
                'id': record.data.id
            },
    ...
    onDelete: function(view, rowIdx, colIdx, item, e, record, row){
        ...
        Ext.Msg.show({
            ...
            fn: function(btn) {
                if (btn === 'yes') {   
                    Ext.Ajax.request({
                        url: '/admin/districts/delete.json',
                        params:{
                            id:record.data.id
                        },
                        methid: 'POST',
...
*/        
    onFormCancel: function() {
        this.lookupReference('districtsForm').getForm().reset();
        this.getView().close();
    },
/*
    Form save function
*/      
    onFormSave: function() {
        ...
        var vcontroller = this;
        var formPanel = this.lookupReference('districtsForm');
