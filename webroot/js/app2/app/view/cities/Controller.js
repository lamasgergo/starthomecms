Ext.define('Tscrm.view.cities.Controller', {
...
*/
    reloadGrid:function(){
        ...
        this.getViewModel().getStore('citiesList').load({
            scope: this,
...
*/
    loadViewData: function(controller,id,win){
        ...
        if(id!='')
        {
            ...
            var data = Ext.data.StoreManager.lookup('CitiesView').load({
                params:{
                    'id': id
                },
                callback: function(records, operation, success) {
...
*/       
    onEdit: function(view, rowIdx, colIdx, item, e, record, row){
        ...
        var formPanel = win.lookupReference('citiesForm');
        formPanel.mask("Adatok betöltése..."); 
        var form = formPanel.getForm(); 
               
        form.url='/admin/cities/edit.json';
        var data = Ext.data.StoreManager.lookup('CitiesView').load({
            params:{
                'id': record.data.id
            },
...
*/        
    onFormCancel: function() {
        this.lookupReference('citiesForm').getForm().reset();
        this.getView().close();
    },
/*
    Form save function
*/      
    onFormSave: function() {
        ...
        var vcontroller = this;
        var formPanel = this.lookupReference('citiesForm');
